...
import { achievementTranslationsEn } from "@/store/achievementStore";
...
export default function AchievementsPage() {
  ...
  const getProgressText = (a: (typeof achievements)[0]) => {
    ...
    if (type === "total_rows") {
      return locale === "ja"
        ? `${Math.min(progress.totalRows, value)}/${value}段`
        : `${Math.min(progress.totalRows, value)}/${value} rows`;
    }
    if (type === "project_count") {
      return locale === "ja"
        ? `${Math.min(progress.projectCount, value)}/${value}個`
        : `${Math.min(progress.projectCount, value)}/${value} items`;
    }
    return "";
  };
  ...
  const getUnlockDate = (unlockedAt: string) => {
    return new Date(unlockedAt).toLocaleDateString(locale === "ja" ? "ja-JP" : "en-US");
  };
  ...
  return (
    <div className="px-4 pt-10 pb-24 max-w-md mx-auto">
      ...
      <div className="bg-white dark:bg-slate-800 border border-gray-100 dark:border-gray-700 rounded-2xl p-5 mb-6 shadow-sm">
        <div className="grid grid-cols-3 gap-4 mb-4">
          <div className="text-center">
            ...
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{t("totalRows")}</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3B82F6] dark:text-blue-400">
              {progress.projectCount}
            </p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{t("projectCount")}</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-[#3B82F6] dark:text-blue-400">
              {unlockedCount}/{totalCount}
            </p>
            <p className="text-xs text-gray-400 dark:text-gray-500 mt-0.5">{t("unlockedCount")}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        {achievements.map((a) => {
          const enTrans = achievementTranslationsEn[a.id];
          const displayTitle = locale === "en" && enTrans ? enTrans.titleEn : a.title;
          const displayDesc = locale === "en" && enTrans ? enTrans.descriptionEn : a.description;
